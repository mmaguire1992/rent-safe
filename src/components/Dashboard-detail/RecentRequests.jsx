...
function RecentRequests({ recentRequests, sliderSettings }) {
    ...
    <div className="bg-white overflow-hidden mt-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="bg-[#E8E2FF] text-[#4A2FCC] text-sm font-bold rounded-full w-5 h-5 md:w-6 md:h-6 flex items-center justify-center">
            4
          </span>
        </div>
        <button className="text-[#2177CE] font-bold font-nunito text-sm">
          View All
        </button>
      </div>
            ...
            {recentRequests.map((request) => (
              <div key={request.id}>
                <div
                  ...
                  className="border border-lightGray rounded-[20px]  transition-colors h-full bg-white mx-0 cursor-pointer"
                >
