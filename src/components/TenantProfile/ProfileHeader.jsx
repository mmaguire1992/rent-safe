...
function ProfileHeader({ tenantData, onSendOffer }) {
  ...
  return (
    <div className="bg-lightGrayGradient rounded-[20px] shadow-[0px_0px_40px_0px_#4A4A4A14] border border-lightGray p-6">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
        <div className="relative">
          <img
            src={tenantData.profileImage}
            alt={tenantData.name}
            className="w-24 h-24 md:w-[150px] md:h-[150px] shadow-[0px_0px_40px_0px_#4A4A4A14] rounded-full object-cover"
          />
          {tenantData.verified && (
