...
function PropertiesTable({
  ...
  const portalPortalRef = useRef(null);

  // Close dropdown when clicking outside
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (openDropdownId) {
        const isClickInsideTrigger = dropdownRefs.current[openDropdownId] && dropdownRefs.current[openDropdownId].contains(event.target);
        const isClickInsidePortal = portalPortalRef.current && portalPortalRef.current.contains(event.target);
  ...
  const handleToggleDropdown = (propertyId, e) => {
    if (openDropdownId === propertyId) {
      setOpenDropdownId(null);
    } else {
      // Calculate position
      if (e && e.currentTarget) {
        const rect = e.currentTarget.getBoundingClientRect();
        // Position: top = bottom of button, left = right of button - width of dropdown (128px)
        const top = rect.bottom + 5; // 5px gap
        const left = rect.right - 128; // Align right edge

        setDropdownPosition({ top, left });
      } else if (dropdownRefs.current[propertyId]) {
        // Fallback if event invalid but ref exists
        const rect = dropdownRefs.current[propertyId].getBoundingClientRect();
        const top = rect.bottom + 5;
        const left = rect.right - 128 - 10; // Extra adjust if needed
        setDropdownPosition({ top, left });
      }

      setOpenDropdownId(propertyId);
    }
  };
                    ...
                    >
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold font-nunito text-secondary text-base truncate lg:w-[170px]">
                              {property.description}
                            </p>
                            <p className="text-darkGray text-base font-nunito font-normal truncate lg:w-[170px]">
                              ID: {property.id}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="py-4 px-4">
                        <p className="text-secondary  text-base font-nunito font-normal truncate lg:w-[200px]">
                          {property.location}
                        </p>
                      </td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4 relative">
                        ...
                        >
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleToggleDropdown(property.id, e);
                            }}
                          ...
                          {openDropdownId === property.id &&
                            createPortal(
                              <div
                                ref={portalPortalRef}
                                className="fixed bg-white border border-lightGray rounded-lg shadow-lg z-[9999] w-32"
                                style={{
                              ...
                              >
                                <button
                                  ...
                                  className="flex items-center gap-2 w-full text-left px-4 py-2 text-base text-darkGray font-nunito font-medium hover:bg-gray-50 first:rounded-t-lg"
                                >
                                  <FiEdit size={16} />
                                  Edit
                                </button>
                                <button
                                  ...
                                  className="flex items-center gap-2 w-full text-left px-4 py-2 text-base text-darkGray font-nunito font-medium hover:bg-gray-50"
                                >
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-2 w-full text-left px-4 py-2 text-base text-darkGray font-nunito font-medium hover:bg-gray-50 last:rounded-b-lg"
                                >
                                  <FiShare2 size={16} />
                                  Share
                                </button>
